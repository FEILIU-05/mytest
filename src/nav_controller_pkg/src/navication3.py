...
# -*- coding: utf-8 -*-
"""
navigation_controller.py  – V1.6.0 (Refactored)
----------------------------------
...
class QRCodeDetector:
    """二维码检测器"""
    def __init__(self, audio_config): # 修改：接收音频配置
        self.bridge = CvBridge()
...
class Navigator(object):
    def __init__(self, csv_path, tuning_path, config_path):
        ...
        self.app_config = self.load_yaml(config_path) # 新增：加载主配置文件
        ...
        self.traffic_light_detector = TrafficLightDetector() # 此类暂无外部配置
        ...
        if detected and object_name:
            ...
            if object_name in target_classes:
                if object_name not in self.detected_objects:
                    ...
                    rospy.loginfo('📝 已记录物体: %s. 当前列表: %s', object_name, self.detected_objects)
                    return {'status': 'success', 'object_name': object_name, 'target_found': True}
                else:
                    rospy.loginfo('⚠️ 物体 %s 已经检测过了', object_name)
                    return {'status': 'duplicate'}
            else:
                rospy.loginfo('⚠️ 检测到的物体 %s 不在当前任务类别中', object_name)
    ...
    def _action_handle_traffic_light(self, point_idx):
        ...
        if green_count > red_count and green_count >= 20:
            ...
            self.play_traffic_light_audio(point_idx)
    ...
    def run(self):
        ...
        traffic_light_logic_finished = False
        skip_to_point = None
        ...
        for idx, wp in enumerate(self.wps, 1):
            ...
            if result:
                if result.get('target_found'):
                    ...
                    skip_to_point = 6 # 跳转到第6个点
                    rospy.loginfo('⏭️ 设置跳转到第6个导航点')
                    self.print_task_summary()
                
                if result.get('status') == 'green_light':
                    if idx == 7:
    ...
    def play_traffic_light_audio(self, point_idx):
        """根据配置播放交通灯语音"""
        audio_cfg = self.app_config.get('audio', {})
        audio_map = audio_cfg.get('traffic_light_map', {})
        filename = audio_map.get(point_idx)
        
        if filename:
            audio_dir = audio_cfg.get('traffic_light_dir', '')
            if not audio_dir:
                rospy.logwarn("⚠️ 交通灯语音目录未在 config.yaml 中配置!")
                return
    ...
    def navigate(self, wp):
        ...
        if not self.wait_for_mb():
            rospy.logerr('❌ 粗导航失败 (tag=%s)', wp['tag'])
            # 根据需要决定是否要在此处停止整个程序
            return
        rospy.loginfo('✔ 粗导航完成')
        # 仅对 'frame' 标签的点进行精调
    ...
    def refine(self, goal):
        ...
        while (rospy.Time.now() - t0).to_sec() < self.tuning_cfg['refine_timeout']:
            ...
            # 如果在计算误差时返回了最大距离，说明TF变换失败，应停止精调
            if dist >= 999:
                rospy.logerr("精调时TF变换失败，已终止。")
                break
    ...
    def cmd_rot(self, yaw_err):
        ...
        cmd = Twist()
        if abs(w) > self.tuning_cfg.get('min_ang', 0.1):
...
if __name__ == '__main__':
    rospy.init_node('navigator_v160_refactored', anonymous=False)
    ...
    try:
        ...
        if hasattr(nav, 'app_config') and nav.app_config:
            ...
            rospy.logerr("Navigator 对象初始化失败，程序退出。")
