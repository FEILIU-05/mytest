...
class UcarCamera:
    def __init__(self):
        ...
        rospy.loginfo("初始化 UcarCamera 节点 (实时检测模式)...")
        ...
        # 发布交通灯状态
        self.traffic_light_pub = rospy.Publisher('/traffic_light_status', String, queue_size=10)  # 增大队列大小
    ...
    def image_callback(self, msg):
        try:
            ...
            if not self.is_initialized:
                ...
                rospy.loginfo("交通灯识别系统已激活，开始实时输出检测结果")

            # 交通灯检测
            current_status = self.detect_traffic_light(rgb_frame)

            # 实时发布每帧的检测结果
            self.publish_status(current_status)
            
            # 仅在状态变化时打印日志（避免日志刷屏）
            if current_status != self.last_status:
                rospy.loginfo("检测到交通灯状态变化: %s" % current_status)
