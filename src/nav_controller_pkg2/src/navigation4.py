...
# -*- coding: utf-8 -*-
"""
navigation_controller.py  – V1.7.4 (Robust Unicode Encoding)
----------------------------------
...
class ObjectDetector:
    ...
    def run_object_detection(self):
        ...
        start_time, detection_timeout = rospy.Time.now(), 20.0 # Increased timeout
...
class Navigator(object):
    ...
    def _action_handle_traffic_light(self, point_idx):
        ...
        while (rospy.Time.now() - start_time).to_sec() < detection_time and not rospy.is_shutdown():
            ...
            rospy.sleep(0.1)
        rospy.loginfo((u'📋 检测完成，统计: 绿灯 %d, 红灯 %d' % (green_count, red_count)).encode('utf-8'))
        if green_count > red_count and green_count >= 20:
            ...
            rospy.loginfo(u'❌ 判定为红灯或无信号'.encode('utf-8'))
    ...
    def run(self):
        ...
        for idx, wp in enumerate(self.wps, 1):
            ...
            if traffic_light_logic_finished and action_name == 'handle_traffic_light':
                rospy.loginfo(u"ℹ️ 交通灯逻辑已完成，跳过检测。".encode('utf-8'))
                continue
            ...
            if result:
                if result.get('target_found'):
                    ...
                    jump_target = self.app_config.get('navigation_flow', {}).get('object_detection_jump_target', 6)
                    skip_to_point = jump_target
                    rospy.loginfo((u'⏭️ 目标已找到，从配置加载跳转目标，将跳转到第%d个导航点' % jump_target).encode('utf-8'))
                
                if result.get('status') == 'green_light' and idx == 7:
                    traffic_light_logic_finished = True
                    skip_to_point = len(self.wps) + 1
                    rospy.loginfo(u'⏭️ 绿灯通过，任务结束'.encode('utf-8'))
...
if __name__ == '__main__':
    rospy.init_node('navigator_v174_robust_unicode', anonymous=False)
    ...
    try:
        ...
        nav = Navigator(
            ...
            config_path=os.path.join(root, 'config2.yaml')
        )
